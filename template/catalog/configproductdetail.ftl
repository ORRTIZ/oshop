      ...
      <form method="post" action="<@ofbizUrl>additem<#if requestAttributes._CURRENT_VIEW_??>/${requestAttributes._CURRENT_VIEW_}</#if></@ofbizUrl>" name="addform" style='margin: 0;'>
        ...
        <#else>
          <#if inStock>
            ...
            <#else>
              <a href="javascript:addItem()" class="btn btn-warning btn-sm"><span style="white-space: nowrap;">${uiLabelMap.OrderAddToCart}</span></a>&nbsp;
...
<#-- Upgrades/Up-Sell/Cross-Sell -->
  ...
  <#if assocProducts?has_content>
    <tr><td>&nbsp;</td></tr>
    <tr><td colspan="2"><h3>${beforeName!}<#if showName == "Y">${productContentWrapper.get("PRODUCT_NAME", "html")!}</#if>${afterName!}</h3></td></tr>
...
<#if commonFeatureResultIds?has_content>
  <h3>Similar Products That Might Interest You...</h3>
