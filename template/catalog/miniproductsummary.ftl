...
<#if miniProduct??>
    <a href="<@ofbizCatalogAltUrl productId=miniProduct.productId/>" class="linktext">${miniProductContentWrapper.get("PRODUCT_NAME", "html")?default("No Name Available")}</a>
        ...
        ${miniProduct.productId}
          <#if (priceResult.price?default(0) > 0 && miniProduct.requireAmount?default("N") == "N")>
            ...
            <#if totalPrice??>
              ${uiLabelMap.ProductAggregatedPrice}: <span class='basePrice'><@ofbizCurrency amount=totalPrice isoCode=priceResult.currencyUsed/></span>
            <#else>
    ...
    <#elseif miniProduct.isVirtual?default("N") == "Y">
        <a href="<@ofbizCatalogAltUrl productCategoryId=requestParameters.category_id?? productId=miniProduct.productId/>" class="btn btn-default btn-xs"><span style="white-space: nowrap;">${uiLabelMap.OrderChooseVariations}...</span></a>
    <#elseif miniProduct.requireAmount?default("N") == "Y">
        <a href="<@ofbizCatalogAltUrl productCategoryId=requestParameters.category_id?? productId=miniProduct.productId/>" class="btn btn-default btn-xs"><span style="white-space: nowrap;">${uiLabelMap.OrderChooseAmount}...</span></a>
    <#else>
        <form method="post" action="<@ofbizUrl>additem<#if requestAttributes._CURRENT_VIEW_?has_content>/${requestAttributes._CURRENT_VIEW_}</#if></@ofbizUrl>" name="${miniProdFormName}" style="margin: 0;">
          <fieldset>
            ...
            <a href="javascript:document.${miniProdFormName}.submit()" class="btn btn-default btn-xs"><span style="white-space: nowrap;">${uiLabelMap.CommonAdd} ${miniProdQuantity} ${uiLabelMap.OrderToCart}</span></a>
          </fieldset>
        </form>
    </#if>
