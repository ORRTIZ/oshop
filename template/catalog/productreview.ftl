...
<#if requestParameters.product_id??>
  <form id="reviewProduct" method="post" action="<@ofbizUrl>createProductReview</@ofbizUrl>">
    ...
    <fieldset class="inline">
      ...
      <div>
        <label for="review">${uiLabelMap.CommonComment}:</label>
        <textarea class="textAreaBox" name="productReview" cols="40"></textarea>
      </div>
      <div class="row">
        <div class="col-md-6 text-left">
        <a href="${productUrl}" class="btn btn-warning btn-sm">${uiLabelMap.CommonCancel}</a>
        </div>
        <div class="col-md-6 text-right">
        <a href="javascript:document.getElementById('reviewProduct').submit();" class="btn btn-success btn-sm">${uiLabelMap.CommonSave}</a>
      </div>
    </fieldset>
  </form>
    </div>
